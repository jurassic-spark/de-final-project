...
resource "aws_lambda_permission" "allow_ingestion_bucket_to_invoke_transform_lambda" {
    ...
    source_arn = aws_s3_bucket.ingestion_zone.arn # restricts permissions to my specific processed bucket
}

# trigger lambda when new files are added to the processed bucket
